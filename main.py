...
# --- UI Components (ScreenSelector updated) ---
class ScreenSelector(QWidget):
    def __init__(self, parent=None, callback=None):
        super().__init__(parent)
        self.callback = callback if callback else self.parent().capture_and_ocr
        self.setWindowFlags(Qt.WindowType.FramelessWindowHint | Qt.WindowType.WindowStaysOnTopHint)
        self.setAttribute(Qt.WidgetAttribute.WA_TranslucentBackground); self.setCursor(Qt.CursorShape.CrossCursor)
        screen_geometry = QApplication.primaryScreen().geometry(); self.setGeometry(screen_geometry)
        self.rubber_band = QRubberBand(QRubberBand.Shape.Rectangle, self); self.origin = QPoint()
    ...
    def mouseReleaseEvent(self, event):
        self.hide(); QApplication.instance().processEvents()
        selected_rect = self.rubber_band.geometry()
        self.callback(selected_rect) # Use the callback
        self.close()

# ... (All other UI component classes are unchanged) ...
...
class ActionEditorDialog(QDialog):
    def __init__(self, action=None, parent=None):
        super().__init__(parent); self.setWindowTitle("Chỉnh sửa Hành động"); layout = QFormLayout(self); self.action_type = QComboBox(); self.action_type.addItems(["Nhấp vào", "Gõ vào", "Trích xuất vào Biến"]); self.target_text = QLineEdit(); self.value_label = QLabel("Giá trị:"); self.value_text = QLineEdit(); self.action_type.currentTextChanged.connect(self.update_labels); layout.addRow("Hành động:", self.action_type); layout.addRow("Tìm văn bản:", self.target_text); layout.addRow(self.value_label, self.value_text)
        if action: self.action_type.setCurrentText(action.get("type", "Nhấp vào")); self.target_text.setText(action.get("target", "")); self.value_text.setText(action.get("value", ""))
        self.update_labels(self.action_type.currentText())
    ...
    def update_labels(self, action_type):
        if action_type == "Nhấp vào": self.value_label.setVisible(False); self.value_text.setVisible(False)
        elif action_type == "Gõ vào": self.value_label.setText("Văn bản cần gõ:"); self.value_label.setVisible(True); self.value_text.setVisible(True)
        elif action_type == "Trích xuất vào Biến": self.value_label.setText("Tên biến (ví dụ: $data):"); self.value_label.setVisible(True); self.value_text.setVisible(True)
    def get_action(self): return {"type": self.action_type.currentText(), "target": self.target_text.text(), "value": self.value_text.text()}
class ProcessEditorDialog(QDialog):
    ...
    def refresh_actions_list(self):
        ...
        for i, action in enumerate(self.process["actions"]): self.actions_list.addItem(f"{i+1}. {action['type']}: Tìm '{action['target']}' - Giá trị: '{action.get('value', '')}'")
...
class AutomationEngine(QThread):
    ...
    def __init__(self, parent=None): super().__init__(parent); self.running = True
    def get_active_window_title(self):
        try: command = "xdotool getactivewindow getwindowname"; result = subprocess.run(command, shell=True, check=True, stdout=subprocess.PIPE, text=True); return result.stdout.strip()
...
class ProcessRunner(QThread):
    log_message = pyqtSignal(str); request_highlight = pyqtSignal(QRect); request_suggestion = pyqtSignal(str, QPoint); request_correction = pyqtSignal(dict, str) # Added process_name
    def __init__(self, process, ocr_df, parent=None):
        super().__init__(parent); self.process = process; self.ocr_df = ocr_df; self.variables = {}; self.continue_event = QEventLoop(); self.corrected_action = None
    ...
    def run(self):
        ...
        for i, action in enumerate(self.process['actions']):
            self.log_message.emit(f"Bước {i+1}: {action['type']} '{action['target']}'"); target_rect = self.find_text_location(action['target'])
            if not target_rect:
                self.log_message.emit(f"Lỗi: Không tìm thấy '{action['target']}' trên màn hình."); self.request_correction.emit(action, self.process['name']); self.continue_event.exec()
                if self.corrected_action:
                    self.log_message.emit("Đã nhận được chỉnh sửa. Thử lại..."); action = self.corrected_action; self.process['actions'][i] = action; target_rect = self.find_text_location(action['target'])
            ...
            if self.process['mode'] == "Tự động": self.execute_action_auto(action, target_rect)
            else: self.execute_action_assist(action, target_rect); break
            time.sleep(1)
        self.log_message.emit("Hoàn thành quy trình.")
    def execute_action_auto(self, action, rect):
        center_pos = rect.center(); action_type = action['type']; value = action.get('value', '')
        if value.startswith('$'): value = self.variables.get(value, '')
        if action_type == "Nhấp vào": pyautogui.click(center_pos.x(), center_pos.y())
        elif action_type == "Gõ vào": pyautogui.click(center_pos.x() + rect.width(), center_pos.y()); pyautogui.write(value, interval=0.05)
        elif action_type == "Trích xuất vào Biến": self.log_message.emit(f"Placeholder: Trích xuất dữ liệu cho biến '{value}'")
    def execute_action_assist(self, action, rect):
        self.request_highlight.emit(rect); action_type = action['type']; suggestion_text = ""
        if action_type == "Nhấp vào": suggestion_text = f"Bây giờ, hãy nhấp vào '{action['target']}'"
        elif action_type == "Gõ vào": suggestion_text = f"Bây giờ, hãy gõ '{action['value']}' vào ô bên cạnh '{action['target']}'"
        elif action_type == "Trích xuất vào Biến": suggestion_text = f"Dữ liệu bên cạnh '{action['target']}' sẽ được lưu vào '{action['value']}'"
        self.request_suggestion.emit(suggestion_text, rect.topRight())

# --- Main Window ---
class FloatingMenu(QMainWindow):
    def __init__(self):
        super().__init__(); self.notes = []; self.rules = []; self.processes = []; self.recently_triggered = {}; self.highlight_windows = []
        ...
        self.extract_button = QPushButton("Trích xuất văn bản"); self.create_note_button = QPushButton("Tạo Ghi chú"); self.rules_button = QPushButton("Quản lý Quy tắc"); self.processes_button = QPushButton("Quản lý Quy trình")
        layout.addWidget(self.extract_button); layout.addWidget(self.create_note_button); layout.addWidget(self.rules_button); layout.addWidget(self.processes_button)
        ...
        self.process_manager = ProcessManagerWindow(self); self.rule_manager = RuleManagerWindow(self, processes=self.process_manager.processes)
        self.extract_button.clicked.connect(self.start_selection); self.create_note_button.clicked.connect(self.create_note); self.rules_button.clicked.connect(self.rule_manager.exec); self.processes_button.clicked.connect(self.process_manager.exec)
        self._drag_start_position = None; self.selector = None
        self.load_notes(); self.on_rules_changed(self.rule_manager.rules); self.on_processes_changed(self.process_manager.processes)
        self.automation_engine = AutomationEngine(self); self.automation_engine.scan_complete.connect(self.process_screen_data); self.automation_engine.start()
    ...
    def process_correction(self, rect, failed_action, process_name):
        ...
        confirm_msg.setText(f"Bạn có muốn cập nhật hành động để tìm văn bản mới '{new_text}' không?")
        ...
        if confirm_msg.exec() == QMessageBox.StandardButton.Yes:
            ...
            corrected_action['target'] = new_text
            # Find and update the process permanently
            for p in self.processes:
                if p['name'] == process_name:
                    for i, act in enumerate(p['actions']):
                        if act['target'] == failed_action['target']: # Simple check
